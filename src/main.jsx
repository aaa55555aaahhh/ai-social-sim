...
function App() {
  ...
  return <div className="phone-app">
    ...
    {systemOpen && <System onClose={() => setSystemOpen(false)} />}
  </div>
}
...
function System({ onClose }) { const [world, setWorld] = useState(() => localStorage.getItem('nocturne-world') || '成年人之间的虚构社交与关系模拟。'); return <div className="system-layer"><div className="system-sheet"><div className="sheet-head"><div><span className="app-kicker">GLOBAL</span><h2>系统设定</h2></div><button className="top-icon" onClick={onClose}><X size={18}/></button></div><label>世界观<textarea rows="5" value={world} onChange={e => setWorld(e.target.value)} /></label><label>AI API Endpoint<input placeholder="后续接入时填写" /></label><button className="lime-button" onClick={() => { localStorage.setItem('nocturne-world', world); onClose() }}>保存设置</button><p className="privacy-note">当前为本地模拟模式。所有人物与聊天内容均为虚构。</p></div></div> }
